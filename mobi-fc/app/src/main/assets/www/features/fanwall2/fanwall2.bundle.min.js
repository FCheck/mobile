...
return FanwallUtils.postModal()};GoogleMaps.init();Fanwall.loadSettings().then(function(){$scope.getCardDesign();$scope.getSettings();$scope.settingsAreLoaded=!0;if(Fanwall.initPostId!==null){Loader.show($translate.instant('Loading post...','fanwall'));FanwallPost.findOne(Fanwall.initPostId).then(function(payload){FanwallUtils.showPostModal(payload.collection)},function(error){Loader.hide();Dialog.alert('Sorry','The post you are looking for is not available!','Dismiss',-1,'fanwall')}).then(function(){Loader.hide();Fanwall.initValuetId=null;Fanwall.initPostId=null})}});$rootScope.$on('fanwall.pageTitle',function(event,payload){$timeout(function(){$scope.pageTitle=payload.pageTitle})})});angular.module('starter').controller('FanwallPostController',function($ionicScrollDelegate,$rootScope,$scope,$state,$stateParams,$timeout,FanwallPost){angular.extend($scope,{isLoading:!1,collection:[],hasMore:!1});$scope.loadMore=function(){$scope.loadContent(!1,!0)};$scope.loadContent=function(refresh,loadMore){if(refresh===!0){$scope.isLoading=!0;$scope.collection=[];$timeout(function(){$ionicScrollDelegate.$getByHandle('mainScroll').scrollTop()})}
...
$scope.form.date=Math.round(Date.now()/1000);$scope.instantAppend($scope.form.text);$scope.isSending=!0;return FanwallPost.sendComment($scope.post.id,null,$scope.form).then(function(payload){$scope.clearComment();$timeout(function(){$scope.post.comments=angular.copy(payload.comments);$scope.post.commentCount=$scope.post.comments.length})},function(payload){Dialog.alert('Error',payload.message,'OK',-1,'OK');$scope.post.comments.pop()}).then(function(){$scope.isSending=!1})}}}});angular.module('starter').directive('fanwallPostList',function($timeout,Lightbox){return{restrict:'E',replace:!0,templateUrl:'features/fanwall2/assets/templates/l1/tabs/directives/post-list.html',link:function(scope){scope.$watch('post',function(){scope._post=scope.post})},controller:function($scope){$scope.listDidRender=function(){$timeout(function(){Lightbox.run('.list-posts')},200)}}}});angular.module('starter').directive('fanwallPostItem',function($rootScope,$filter,$sce,$translate,$timeout,$q,Application,Customer,Dialog,Loader,Fanwall,FanwallPost,FanwallUtils,Lightbox,Popover,SocialSharing){return{restrict:'E',templateUrl:'features/fanwall2/assets/templates/l1/tabs/directives/post-item.html',controller:function($scope){$scope.actionsPopover=null;$scope.popoverItems=[];$scope.post.isFull=!1;$scope.getCardDesign=function(){return Fanwall.getSettings().cardDesign};$scope.getSettings=function(){return Fanwall.getSettings()};$scope.userLike=function(){return $scope.getSettings().features.enableUserLike};$scope.userComment=function(){return $scope.getSettings().features.enableUserComment};$scope.userShareBig=function(){return['big','both'].indexOf($scope.getSettings().features.enableUserShare)>=0};$scope.userShareSmall=function(){return['small','both'].indexOf($scope.getSettings().features.enableUserShare)>=0};$scope.getColSizeTextual=function(){var counter=0;if($scope.userLike()){counter++}
if($scope.userComment()){counter++}
switch(counter){default:case 0:case 1:return'col-100';case 2:return'col-50'}};$scope.getColSize=function(){var counter=0;if($scope.userLike()){counter++}
if($scope.userComment()){counter++}
if($scope.userShareBig()){counter++}
switch(counter){default:case 0:case 1:return'col-100';case 2:return'col-50';case 3:return'col-33'}};$scope.photoMode=function(){return $scope.getSettings().photoMode};$scope.photoPosition=function(){return $scope.getSettings().photoPosition};$scope.showText=function(){return $filter('linky')($scope.post.text)};$scope.showLikeOrComment=function(){return($scope.post.likeCount>0||$scope.post.commentCount>0)&&($scope.canLikeOrComment())};$scope.canLikeOrComment=function(){return($scope.userLike()||$scope.userComment())};$scope.getMaxBodySize=function(){return $scope.getSettings().maxBodySize};$scope.textIsCut=function(){var maxLength=$scope.getMaxBodySize();return maxLength>0&&$scope.post.text.length>maxLength};$scope.cutBody=function(){var maxLength=$scope.getMaxBodySize();if(maxLength<=0||$scope.post.text.length<=maxLength){return $scope.post.text}
return $scope.post.text.substring(0,maxLength)+'...'};$scope.imagePath=function(path){if(path!==undefined){return IMAGE_URL+'images/application'+path}
...
return $q.resolve()};$scope.sharePost=function(){var shareLink=['https://',Application.application.share_domain,'/',APP_KEY,'/fanwall/post/',Fanwall.lastValueId,'/',$scope.post.id].join('');SocialSharing.share('',$translate.instant('Check this post!','fanwall'),'',shareLink)};$scope.flagPost=function(){var title=$translate.instant('Report this message!','fanwall');var message=$translate.instant('Please let us know why you think this message is inappropriate.','fanwall');var placeholder=$translate.instant('Your message.','fanwall');Dialog.prompt(title,message,'text',placeholder,['OK','CANCEL'],-1,'fanwall').then(function(value){Loader.show();FanwallPost.reportPost($scope.post.id,value).then(function(payload){Dialog.alert('Thanks!',payload.message,'OK',2350,'fanwall')},function(payload){Dialog.alert('Error!',payload.message,'OK',-1,'fanwall')}).then(function(){Loader.hide()})})};$scope.buildPopoverItems=function(){var viewHistory={label:$translate.instant('View edit history','fanwall'),icon:'icon ion-clock',click:function(){$scope.closeActions().then(function(){$scope.showHistory()})}};if($scope.userShareSmall()){$scope.popoverItems.push({label:$translate.instant('Share','fanwall'),icon:'icon ion-sb-share-filled',click:function(){$scope.closeActions().then(function(){$scope.sharePost()})}})}
if($scope.isOwner()){$scope.popoverItems.push({label:$translate.instant('Edit post','fanwall'),icon:'icon ion-edit',click:function(){$scope.closeActions().then(function(){$scope.editPost()})}});if($scope.post.history.length>0){$scope.popoverItems.push(viewHistory)}
...
return $q.resolve()};$scope.buildPopoverItems=function(){$scope.popoverItems=[];$scope.popoverItems.push({label:$translate.instant('Unblock user','fanwall'),icon:'icon ion-android-remove-circle',click:function(){$scope.closeActions().then(function(){$scope.unblockUser()})}})};$scope.buildPopoverItems()}}});angular.module('starter').directive('fanwallSlider',function(){return{restrict:'E',scope:{images:'='},templateUrl:'./features/fanwall2/assets/templates/l1/directives/slider.html',controller:function($scope,$ionicGesture,$ionicScrollDelegate,Modal){$scope.carouselIndex=0;$scope.carouselIndexModal=0;$scope.getImageSrc=function(path){if(path!==undefined){return IMAGE_URL+'images/application'+path}
return'./features/fanwall2/assets/templates/images/placeholder.png'};$scope.setCarouselIndex=function(index){var localIndex=index;if(localIndex<0){localIndex=0}else if(localIndex>=$scope.images.length){localIndex=localIndex-1}
$scope.carouselIndexModal=localIndex};$scope.showFullscreen=function(index){Modal.fromTemplateUrl('zoom-modal.html',{scope:$scope,animation:'block'}).then(function(modal){$scope.modal=modal;$scope.carouselIndexModal=index;$scope.modal.show();$scope.modal.is_zoomed=!1;$scope.modal.scale_to_original_size=!1;var element=angular.element(document.getElementById('zoomedImage'));$scope.modal.release=$ionicGesture.on('release',function(){if($scope.modal.scale_to_original_size){$ionicScrollDelegate.$getByHandle('slide-'+$scope.carouselIndexModal).zoomTo(1,!0);$scope.modal.scale_to_original_size=!1}},element);$scope.modal.pinch=$ionicGesture.on('pinch',function(event,o,t,l){if(event.gesture.scale<1){$scope.modal.scale_to_original_size=!0}},element);$scope.modal.doubleTap=$ionicGesture.on('doubletap',function(event){if($scope.modal.is_zoomed){$ionicScrollDelegate.$getByHandle('slide-'+$scope.carouselIndexModal).zoomTo(1,!0)}else{$ionicScrollDelegate.$getByHandle('slide-'+$scope.carouselIndexModal).zoomTo(3,!0,event.gesture.touches[0].pageX,event.gesture.touches[0].pageY)}
$scope.modal.is_zoomed=!$scope.modal.is_zoomed},element)})};$scope.hideFullscreen=function(){$scope.carouselIndex=$scope.carouselIndexModal;$scope.modal.remove()}}}});angular.module('starter').factory('Fanwall',function($pwaRequest,$stateParams,$state,$rootScope,Application){var factory={storage:[],lastValueId:null,initValueId:null,initPostId:null};factory.loadSettings=function(){if(factory.storage.hasOwnProperty($stateParams.value_id)){return $pwaRequest.resolve(factory.storage[$stateParams.value_id])}
...
var promise=$pwaRequest.get('fanwall/mobile_post/find-all',angular.extend({urlParams:{value_id:$stateParams.value_id,offset:offset},refresh:refresh},factory.extendedOptions));promise.then(function(success){factory.storage[storageKey]=success});return promise};factory.findOne=function(postId){return $pwaRequest.get('fanwall/mobile_post/find-one',angular.extend({urlParams:{value_id:$stateParams.value_id,postId:postId},refresh:!0},factory.extendedOptions))};factory.findAllNearby=function(location,offset,refresh){var storageKey=[$stateParams.value_id,'findAllNearby',location.latitude,location.longitude,offset].join('_');if(refresh!==!0&&factory.storage.hasOwnProperty(storageKey)){return $pwaRequest.resolve(factory.storage[storageKey])}
