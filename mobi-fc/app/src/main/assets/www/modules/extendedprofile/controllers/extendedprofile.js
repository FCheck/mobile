...
App.factory('ExtendedProfile', function($rootScope, $http, Url, Application, Push) {
...
App.config(function($stateProvider) {
    ...
    $scope.post = function() {
        ...
        ExtendedProfile.post($scope.formData).success(function(data) {
            //$scope.formData = {};
            //$scope.preview_src = {};
            if(data.success) {
	  ...
	  $rootScope.$on(AUTH_EVENTS.loginSuccess, function() {
	        HomepageLayout.getFeatures().then(function (features) {
	            ...
	            for(var i = 0; i <  features.data.pages.length; i++){
	            	...
	            	if(features.data.pages[i].code=="extendedprofile" ){

	                //if(features.data.pages[i].code=="extendedprofile" &&  features.data.pages[i].is_active == true ){
	                			var redirectProfilePath = features.data.pages[i].path;
	                			ExtendedProfile.value_id = features.data.pages[i].value_id;
	                			
	                			
	                			
	                			var promise = $pwaRequest.post('customer/mobile_account_edit/find', {
						            cache: false
						        });
						
						        promise
						            .then(function (customer) {
						                console.log(customer.metadatas);
		                				console.log("$window.top"+$window.top+"==$window.self"+$window.self);
							            if($window.top==$window.self && (_.isEmpty(customer.metadatas) || ($rootScope.profileBackend && $rootScope.isLoggedinAsuser))) {
